    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4 sm:space-y-6">
            ...
            <div class="grid grid-cols-3 gap-2 sm:gap-4">
                <div class="bg-white rounded-lg shadow-sm p-3 sm:p-5 text-center">
                    <p class="text-2xl font-bold text-indigo-600">{{ $quizCount ?? 0 }}</p>
                    <p class="text-xs sm:text-sm text-gray-500 mt-1">Quizzes created</p>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-3 sm:p-5 text-center">
                    <p class="text-2xl font-bold text-green-600">{{ $groupCount ?? 0 }}</p>
                    <p class="text-xs sm:text-sm text-gray-500 mt-1">Groups joined</p>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-3 sm:p-5 text-center">
                    <p class="text-2xl font-bold text-gray-700">{{ $topicCount ?? 0 }}</p>
                    <p class="text-xs sm:text-sm text-gray-500 mt-1">Topics created</p>
                </div>
            </div>
                ...
                <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6">
                    ...
                    @forelse($quizzes ?? [] as $quiz)
                        <a href="{{ route('quiz.preview', $quiz->quiz_id) }}"
                           class="flex justify-between items-center gap-3 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 -mx-2 px-2 rounded transition-colors">
                            <div class="min-w-0">
                            ...
                            <span class="text-xs px-2 py-1 rounded-full shrink-0 {{ $quiz->is_published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                    ...
                    <div class="space-y-3">
                        <a href="{{ route('quiz.create') }}"
                           class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 text-sm font-medium text-gray-700">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-indigo-600 shrink-0" fill="none"
                        ...
                        <a href="{{ route('forum.index') }}"
                           class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 text-sm font-medium text-gray-700">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-purple-600 shrink-0"
                                 fill="none" viewBox="0 0 24 24"
                                 stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <a href="{{ route('participation.grade') }}"
                           class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 text-sm font-medium text-gray-700">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-yellow-500 shrink-0"
                                 fill="none" viewBox="0 0 24 24"
                                 stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
