...
<body class="bg-gray-50 min-h-screen">
    ...
    <header class="bg-white border-b border-gray-200 px-6 py-3 flex justify-between items-center sticky top-0 z-10">
        ...
        <div class="flex items-center gap-2 shrink-0 ml-4">
            ...
            {{-- Edit/Delete (Owner or Admin only) — topics.edit is now a real,
                 working route (TopicController@edit/@update), so the Edit
        ...
        <div class="space-y-6">
            @forelse($posts as $post)
                ...
                <div id="post-{{ $post->post_id }}"
                     class="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-indigo-300 transition-all duration-200">
                    ...
                    <div class="p-5">
                        ...
                        {{-- Attachment display — UI only for now. No 'attachment'
                             column exists on posts yet, and PostController never
                             stores an uploaded file, so this block currently has
                             nothing to render. Left in place for whoever finishes
                             wiring the upload feature. --}}
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 z-10">
        <form method="POST"
            ...
            {{-- Not yet functional — see the attachment display comment above --}}
            <input type="file"
                name="attachment"
